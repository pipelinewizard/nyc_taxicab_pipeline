-- nyctaxicab_weekly_summary_by_end_borough
CREATE TABLE nyctaxicab_weekly_summary_by_end_borough AS
SELECT 
    d.week, end_borough_id,
    COUNT(*) AS ride_count,
...
GROUP BY d.week, end_borough_id;
...
UPDATE nyctaxicab_weekly_summary_by_end_borough wseb
SET year = (
    SELECT d.year
    FROM dim_date d
    WHERE d.week = wseb.week
...
SET date_id = (
    SELECT d.date_id
    FROM dim_date d
    WHERE d.date = wseb.week_start_date
)
