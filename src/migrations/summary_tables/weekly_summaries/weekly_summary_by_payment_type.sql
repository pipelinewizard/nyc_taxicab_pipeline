-- nyctaxicab_weekly_summary_bypayment_type
CREATE TABLE nyctaxicab_weekly_summary_bypayment_type AS
SELECT 
    d.week, payment_type_id,
    COUNT(*) AS ride_count,
...
GROUP BY d.week, payment_type_id;

-- Add year and week_start_date Column
ALTER TABLE nyctaxicab_weekly_summary_bypayment_type
ADD COLUMN year INT,
ADD COLUMN week_start_date DATE,
ADD COLUMN date_id INT;

-- Populate year column
UPDATE nyctaxicab_weekly_summary_bypayment_type wspt
SET year = (
    SELECT d.year
    FROM dim_date d
    WHERE d.week = wspt.week
...
SET date_id = (
    SELECT d.date_id
    FROM dim_date d
    WHERE d.date = wspt.week_start_date
)
