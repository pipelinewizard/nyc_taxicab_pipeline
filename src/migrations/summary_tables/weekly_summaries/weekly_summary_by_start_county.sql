-- nyctaxicab_weekly_summary_by_start_county
CREATE TABLE nyctaxicab_weekly_summary_by_start_county AS
SELECT 
    d.week, start_county_id,
    COUNT(*) AS ride_count,
...
GROUP BY d.week, start_county_id;
...
UPDATE nyctaxicab_weekly_summary_by_start_county wssc
SET year = (
    SELECT d.year
    FROM dim_date d
    WHERE d.week = wssc.week
...
SET date_id = (
    SELECT d.date_id
    FROM dim_date d
    WHERE d.date = wssc.week_start_date
)
