-- nyctaxicab_weekly_summary_by_start_borough
CREATE TABLE nyctaxicab_weekly_summary_by_start_borough AS
SELECT 
    d.week, start_borough_id,
    COUNT(*) AS ride_count,
...
GROUP BY d.week, start_borough_id;
...
UPDATE nyctaxicab_weekly_summary_by_start_borough wssb
SET year = (
    SELECT d.year
    FROM dim_date d
    WHERE d.week = wssb.week
...
SET date_id = (
    SELECT d.date_id
    FROM dim_date d
    WHERE d.date = wssb.week_start_date
)
