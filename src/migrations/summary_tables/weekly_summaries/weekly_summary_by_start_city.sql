-- nyctaxicab_weekly_summary_by_start_city
CREATE TABLE nyctaxicab_weekly_summary_by_start_city AS
SELECT 
    d.week, start_city_id,
    COUNT(*) AS ride_count,
...
UPDATE nyctaxicab_weekly_summary_by_start_city wssct
SET year = (
    SELECT d.year
    FROM dim_date d
    WHERE d.week = wssct.week
...
SET date_id = (
    SELECT d.date_id
    FROM dim_date d
    WHERE d.date = wssct.week_start_date
)
