-- nyctaxicab_weekly_summary_by_end_city
CREATE TABLE nyctaxicab_weekly_summary_by_end_city AS
SELECT 
    d.week, end_city_id,
    COUNT(*) AS ride_count,
...
UPDATE nyctaxicab_weekly_summary_by_end_city wsect
SET year = (
    SELECT d.year
    FROM dim_date d
    WHERE d.week = wsect.week
...
SET date_id = (
    SELECT d.date_id
    FROM dim_date d
    WHERE d.date = wsect.week_start_date
)
