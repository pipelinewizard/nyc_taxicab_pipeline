-- nyctaxicab_monthly_summary_by_start_county_id
CREATE TABLE nyctaxicab_monthly_summary_by_start_county AS
SELECT 
    d.month, start_county_id,
    COUNT(*) AS ride_count,
...
GROUP BY d.month, start_county_id;
...
UPDATE nyctaxicab_monthly_summary_by_start_county mssc
SET year = (
    SELECT d.year
    FROM dim_date d
    WHERE d.month = mssc.month
...
SET date_id = (
    SELECT d.date_id
    FROM dim_date d
    WHERE d.date = mssc.month_start_date
)
