-- nyctaxicab_monthly_summary_by_start_city
CREATE TABLE nyctaxicab_monthly_summary_by_start_city AS
SELECT 
    d.month, start_city_id,
    COUNT(*) AS ride_count,
...
GROUP BY d.month, start_city_id;
...
UPDATE nyctaxicab_monthly_summary_by_start_city mssct
SET year = (
    SELECT d.year
    FROM dim_date d
    WHERE d.month = mssct.month
...
SET date_id = (
    SELECT d.date_id
    FROM dim_date d
    WHERE d.date = mssct.month_start_date
)
