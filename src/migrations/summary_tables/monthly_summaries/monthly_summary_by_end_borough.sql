-- nyctaxicab_monthly_summary_by_end_borough
CREATE TABLE nyctaxicab_monthly_summary_by_end_borough AS
SELECT 
    d.month, end_borough_id,
    COUNT(*) AS ride_count,
...
GROUP BY d.month, end_borough_id;
...
UPDATE nyctaxicab_monthly_summary_by_end_borough mseb
SET year = (
    SELECT d.year
    FROM dim_date d
    WHERE d.month = mseb.month
...
SET date_id = (
    SELECT d.date_id
    FROM dim_date d
    WHERE d.date = mseb.month_start_date
)
