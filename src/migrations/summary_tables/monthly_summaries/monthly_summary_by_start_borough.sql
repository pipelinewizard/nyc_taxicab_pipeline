-- nyctaxicab_monthly_summary_by_start_borough
CREATE TABLE nyctaxicab_monthly_summary_by_start_borough AS
SELECT 
    d.month, start_borough_id,
    COUNT(*) AS ride_count,
...
GROUP BY d.month, start_borough_id;
...
UPDATE nyctaxicab_monthly_summary_by_start_borough mssb
SET year = (
    SELECT d.year
    FROM dim_date d
    WHERE d.month = mssb.month
...
SET date_id = (
    SELECT d.date_id
    FROM dim_date d
    WHERE d.date = mssb.month_start_date
)
