-- nyctaxicab_monthly_summary_by_end_state_id
CREATE TABLE nyctaxicab_monthly_summary_by_end_state AS
SELECT 
    d.month, end_state_id,
    COUNT(*) AS ride_count,
...
UPDATE nyctaxicab_monthly_summary_by_end_state mses
SET year = (
    SELECT d.year
    FROM dim_date d
    WHERE d.month = mses.month
...
SET date_id = (
    SELECT d.date_id
    FROM dim_date d
    WHERE d.date = mses.month_start_date
)
