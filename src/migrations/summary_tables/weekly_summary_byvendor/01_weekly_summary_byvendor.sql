...
CREATE TABLE nyctaxicab_weekly_summary_byvendor AS
SELECT 
    d.week, vendor_id,
    COUNT(*) AS ride_count,
...
UPDATE nyctaxicab_weekly_summary_byvendor wsv
SET year = (
    SELECT d.year
    FROM dim_date d
    WHERE d.week = wsv.week
...
SET date_id = (
    SELECT d.date_id
    FROM dim_date d
    WHERE d.date = wsv.week_start_date
)
