-- nyctaxicab_weekly_summary_by_end_state
CREATE TABLE nyctaxicab_weekly_summary_by_end_state AS
SELECT 
    d.week, end_state_id,
    COUNT(*) AS ride_count,
...
UPDATE nyctaxicab_weekly_summary_by_end_state wses
SET year = (
    SELECT d.year
    FROM dim_date d
    WHERE d.week = wses.week
...
SET date_id = (
    SELECT d.date_id
    FROM dim_date d
    WHERE d.date = wses.week_start_date
)
