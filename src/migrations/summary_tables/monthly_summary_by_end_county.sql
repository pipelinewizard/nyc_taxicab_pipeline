-- nyctaxicab_monthly_summary_by_end_county_id
CREATE TABLE nyctaxicab_monthly_summary_by_end_county AS
SELECT 
    d.month, end_county_id,
    COUNT(*) AS ride_count,
...
GROUP BY d.month, end_county_id;

-- Add date_id, year and month_start_date Column
ALTER TABLE nyctaxicab_monthly_summary_by_end_county
ADD COLUMN year INT,
ADD COLUMN month_start_date DATE,
ADD COLUMN date_id INT;

-- Populate year column
UPDATE nyctaxicab_monthly_summary_by_end_county msec
SET year = (
    SELECT d.year
    FROM dim_date d
    WHERE d.month = msec.month
...
SET date_id = (
    SELECT d.date_id
    FROM dim_date d
    WHERE d.date = msec.month_start_date
)
